...
void init(int n, float *alpha, float *beta, float *A, float *u1, float *v1, float *u2, float *v2, float *w, float *x, float *y, float *z) {
    ...
    *beta = 1.0f;
    for (int i = 0; i < n; ++i) {
        u1[i] = static_cast<float>(i % 16) / n;
        u2[i] = static_cast<float>(i % 16) / (2.0f * n);
        v1[i] = static_cast<float>((i % 16) + 1) / (4.0f * n);
        v2[i] = static_cast<float>((i % 32) + 1) / (1.5f * n);
        y[i] = static_cast<float>((i % 32) + 1) / (3.0f * n);
        z[i] = static_cast<float>((i % 64) + 1) / (5.0f * n);
        x[i] = 0.0f;
        w[i] = 0.0f;

        for (int j = 0; j < n; ++j)
            A[i * n + j] = static_cast<float>((i % 16) * (j % 16));
    }
}
